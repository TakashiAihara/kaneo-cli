...
Every push to `main` that passes CI is released as the next release candidate (`v0.2.0` → `v0.2.1-rc.1` → `v0.2.1-rc.2`), so the latest release and `install.sh` always match `main`. A final version is cut by pushing its tag by hand, or by running the release workflow with that tag.
